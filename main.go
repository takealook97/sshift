...
func (s *Server) Validate() error {
	...
	if s.KeyPath != "" {
		...
		if info, err := os.Stat(s.KeyPath); err == nil {
			mode := info.Mode()
			if mode&FilePermOwnerOnly != FilePermOwnerOnly {
...
func createSSHClient(host, user, password, keyPath string) (*ssh.Client, error) {
	...
	if keyPath != "" {
		...
		if info, err := os.Stat(keyPath); err == nil {
			mode := info.Mode()
			// Check if file has owner-only permissions (400 or 600)
			if mode&0077 != 0 {
				return nil, fmt.Errorf("SSH key file %s has loose permissions (%s), should be 400 or 600", keyPath, mode.String())
			}
		}
